...
class Modal extends Component{
    constructor(props){
        ...
        // this.AllMessage=this.databaseConversation.child('messages')
        // this.AllMessage.on('child_added', snap=>{
    ...
    updatetMessage=(event)=>{
        ...
        let arr= {};
        let x = this.state.chat[`${personUid}`].messages;
        console.log("x" , x)
    ...
    render(){
        ...
        const {personUid}=this.state
        console.log( 'cpnverastion', this.state.conversation)
        if(this.state.chat){
            localStorageSender=  localStorage.getItem('sender');
            chatInstance=this.state.chat[localStorageSender]
            console.log(chatInstance.sender['mail'])
        }

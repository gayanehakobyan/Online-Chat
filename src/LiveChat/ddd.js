...
class Modal extends Component {
    ...
    showMessageAgain=()=>{
        ...
        if(personUid){
            let arr = {};
             let x = fire.database().ref().child('conversation/').child(window.location.hostname).child(personUid).child('messages')
             x.on('value', snap => {
                x = snap.val();
                if(x){
                                    ...
                                    <div style={{ height: "100%", width: "100%" }}>
                                        {
                                            this.state.conversation ? (
                                                <div style={{overflow: "auto", maxHeight:'100%', maxWidth:'100%'}} ref={el => this.messageSpace = el}>{Object.keys(this.state.conversation).map(el => <p key={el.timestamp} >{this.state.conversation[el]}</p>)}</div>
                                            ) : (
                                                <div>Loading...</div>
                                            )
                                        }
